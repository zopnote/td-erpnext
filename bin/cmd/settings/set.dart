...
final NatrixCommand settingsSetCommand = NatrixCommand(
  ...
  callback: (options) {
    final Settings _settings = Settings.fromDisk();
    final NatrixSection syntax = NatrixStructure(
      sections: [
        settingsListSection,
        NatrixDefaultTheme.of(options.getContext()).usage,
        NatrixLine(
          text: NatrixText(
            "Please specify a setting and a corresponding value.",
          ),
        ),
      ],
    );
    ...
    final Setting? setting = settings.firstWhereOrNull(
      (e) => e.key == options.args.first,
    );
    if (setting == null) {
      io.newLine(
        text: NatrixText(
          "There isn't a setting with the name \"${options.args.first}\".",
        ),
      );
      return;
    }
    ...
    io.newLine(text: NatrixText("Set ${setting.key} to ${setting.value}."));
    _settings.dump();
  },
);
